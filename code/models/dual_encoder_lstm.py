...
class DualEncoderLSTM(BaseModel):
    ...
    def build_model(self):
        ...
        add_echo = True if self.config.add_echo else False
        ...
        with tf.variable_scope("loss"):
            if add_echo:
                self.labels = self.labels = tf.to_float(tf.concat([tf.ones([num_positives, 1]), tf.zeros([num_negatives + num_positives, 1])], 0))
            else:
