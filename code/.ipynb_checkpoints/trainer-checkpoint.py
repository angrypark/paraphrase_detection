...
class ParaphraseDetectionTrainer(BaseTrainer):
    def __init__(self, sess, preprocessor, data, config, summary_writer):
        super(ParaphraseDetectionTrainer, self).__init__(sess, preprocessor, data, config, summary_writer)
        # get size of data
    ...
    def train_step(self, model, sess):
        batch_A, batch_B, \
        batch_sentence_diff, batch_extra_features, labels = next(self.train_iterator)
        
        cur_batch_length = len(batch_A)

        feed_dict = {model.input_A: batch_A,
